    ...
    <div class="animate-fade-up flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
      ...
      <div class="flex items-center gap-3">
        <div class="glass-card !flex-row !p-3 !px-4 !rounded-full !cursor-default !items-center gap-2">
          <span class="text-xs font-semibold text-gray-700">{{$clients->count()}} clients</span>
        </div>
        <div class="glass-card !flex-row !p-3 !px-4 !rounded-full !cursor-default !items-center gap-2">
          <span class="text-xs font-semibold text-gray-700">{{$workers->count()}} workers</span>
        </div>
      </div>
    </div>
      ...
      <div class="flex flex-col divide-y divide-gray-100">
        ...
        <div class="grid grid-cols-12 gap-4 px-6 py-5 items-center hover:bg-gray-50/60 transition-colors">
          ...
          <div class="col-span-3 flex items-center gap-3">
            ...
            <div class="min-w-0">
              <p class="text-sm font-semibold text-gray-900 truncate">{{$client->user->name}}</p>
              <p class="text-xs text-gray-400">Inscrit le {{$client->created_at}}</p>
            </div>
          </div>
          <div class="col-span-2">
            <p class="text-sm text-gray-500 truncate">{{$client->user->email}}</p>
          </div>
          <div class="col-span-2">
          ...
          <div class="col-span-2 flex items-center justify-end gap-2">
            <a href="{{route('client.show',$client->id)}}"  class="w-8 h-8 rounded-xl border border-gray-200 bg-white flex items-center justify-center hover:border-gray-400 transition-all" title="Voir">
        ...
        <div class="grid grid-cols-12 gap-4 px-6 py-5 items-center hover:bg-gray-50/60 transition-colors">
          ...
          <div class="col-span-3 flex items-center gap-3">
            ...
            <div class="min-w-0">
              <p class="text-sm font-semibold text-gray-900 truncate">{{$worker->user->name}}</p>
              <p class="text-xs text-gray-400">Inscrit le {{$worker->created_at}}</p>
            </div>
          </div>
          <div class="col-span-2">
            <p class="text-sm text-gray-500 truncate">{{$worker->user->email}}</p>
          </div>
          <div class="col-span-2">
          ...
          <div class="col-span-2 flex items-center justify-end gap-2">
            <a href="{{route('worker.show',$worker->id)}}" class="w-8 h-8 rounded-xl border border-gray-200 bg-white flex items-center justify-center hover:border-gray-400 transition-all" title="Voir">
              <svg width="13" height="13" viewBox="0 0 16 16" fill="none"><path d="M1 8s2.5-5 7-5 7 5 7 5-2.5 5-7 5-7-5-7-5z" stroke="#6b7280" stroke-width="1.4"/><circle cx="8" cy="8" r="2" stroke="#6b7280" stroke-width="1.3"/></svg>
            </a >
            <button class="text-xs font-semibold px-3 py-1.5 rounded-full border border-red-200 bg-red-50 text-red-600 hover:bg-red-100 hover:border-red-300 transition-all">
              Bannir
